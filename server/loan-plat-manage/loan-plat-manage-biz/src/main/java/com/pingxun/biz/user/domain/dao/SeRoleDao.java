...
public class SeRoleDao {
    ...
    public SeRole updateRole(SeRole role) {
        final String sql = "update pf_se_role set role=?, description=?, resource_ids=?, available=?,merchant_id=?,type=? where id=?";
        try {
             jdbcTemplate.update(
                sql,
                role.getRole(), role.getDescription(), role.getResourceIdsStr(), role.getAvailable(),role.getMerchantId(),role.getType(), role.getId());
        }catch (org.springframework.dao.DuplicateKeyException e){
            CwException.throwIt("角色编码已存在");
        }
        return role;
