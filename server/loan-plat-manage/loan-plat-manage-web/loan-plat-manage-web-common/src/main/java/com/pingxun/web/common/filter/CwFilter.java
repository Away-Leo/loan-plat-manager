package com.pingxun.web.common.filter;
...
public class CwFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (notNeedLogin(uri)) {
            ...
            //权限判断
            if(!checkSessionTimeout(request, response)){
                if(ObjectHelper.isNotEmpty(CPContext.getContext().getSeUserInfo())&&ObjectHelper.isNotEmpty(CPContext.getContext().getSeUserInfo().getAuthUrls())){
                    if(!CPContext.getContext().getSeUserInfo().getAuthUrls().contains(uri)&&!CPContext.getContext().getSeUserInfo().getType().equalsIgnoreCase("admin")){
                        response.setCharacterEncoding("utf-8");
                        request.setCharacterEncoding("UTF-8");
                        response.setContentType("application/json");
                        response.getWriter().print("{\"success\":false,\"code\":\"999999\",\"message\":\"无此权限\",\"data\":[]}");
                        return;
                    }
                }
            }
...
//                response.setStatus(707);
                ...
                request.setCharacterEncoding("UTF-8");
                response.setContentType("application/json");
                response.getWriter().print("{\"success\":false,\"code\":\"100000\",\"message\":\"未登录\",\"data\":[]}");
                return;
            } else {
                filterChain.doFilter(request, response);
            }
